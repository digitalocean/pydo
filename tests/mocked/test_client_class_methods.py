...
def client_resources(mock_client):
    resources = {
        ...
        if "__" not in r and r[0] != "_" and r not in ["close", "send_request"]
    }
...
def test_resource_class_operations(client_resources, group, operations, patched_methods):
    class_methods = client_resources[group]
    if len(patched_methods) > 0:
