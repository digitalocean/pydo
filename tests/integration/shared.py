...
def wait_for_action(client: GeneratedClient, action_id: int, wait_seconds: int = 5):
...
def wait_for_kubernetes_cluster_create(
    client: GeneratedClient, cluster_id: str, wait_seconds: int = 15
):
...
def with_test_droplet(client: GeneratedClient, public_key: bytes, **kwargs):
...
@contextlib.contextmanager
def with_test_tag(client: GeneratedClient, **kwargs):
...
@contextlib.contextmanager
def with_test_volume(client: GeneratedClient, **kwargs):
...
@contextlib.contextmanager
def with_test_kubernetes_cluster(client: GeneratedClient, **kwargs):
...
@contextlib.contextmanager
def with_ssh_key(client: GeneratedClient, public_key) -> str:
