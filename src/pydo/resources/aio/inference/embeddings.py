...
class Embeddings:
    ...
    async def create(self, **kwargs: Any) -> Any:
        """Delegates to ``client.inference.create_embedding``."""
        body = {k: v for k, v in kwargs.items() if k not in _SDK_KWARGS}
        sdk = {k: v for k, v in kwargs.items() if k in _SDK_KWARGS}
        body = _prepare_body("create_embedding", body)
        return _wrap_with_id_alias(
            await self._client.inference.create_embedding(body, **sdk),
            primary_id_field="embedding_id",
        )
