...
class Files:
    ...
    async def create(self, **kwargs: Any) -> Any:
        """Delegates to ``client.inference.create_batch_file``."""
        body = {k: v for k, v in kwargs.items() if k not in _SDK_KWARGS}
        sdk = {k: v for k, v in kwargs.items() if k in _SDK_KWARGS}
        body = _prepare_body("create_batch_file", body)
        return _wrap_with_id_alias(
            await self._client.inference.create_batch_file(body, **sdk),
            primary_id_field="batch_file_id",
        )
