...
class Messages:
    ...
    async def create(self, **kwargs: Any) -> Any:
        """Delegates to ``client.inference.create_messages``."""
        body = {k: v for k, v in kwargs.items() if k not in _SDK_KWARGS}
        sdk = {k: v for k, v in kwargs.items() if k in _SDK_KWARGS}
        body = _prepare_body("create_messages", body)
        return _wrap_with_id_alias(
            await self._client.inference.create_messages(body, **sdk),
            primary_id_field="messages_id",
        )
